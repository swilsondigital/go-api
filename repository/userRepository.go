...
func (u userRepository) FindUserById(id string) (user models.User, err error) {
	err = u.DB.Preload(clause.Associations).Preload("PortfolioRecords.Technologies").Preload("Profile.Technologies").Where("id = ?", id).First(&user).Error
	return user, err
}
...
func (u userRepository) FindUserByEmail(email string) (user models.User, err error) {
	err = u.DB.Preload(clause.Associations).Preload("PortfolioRecords.Technologies").Preload("Profile.Technologies").Where("email = ?", email).First(&user).Error
	return user, err
}
