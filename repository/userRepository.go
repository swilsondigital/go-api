...
func (u userRepository) FindUserById(id string) (user models.User, err error) {
	err = u.DB.Preload(clause.Associations).Preload("PortfolioRecords.Technologies").Preload("Profile.Technologies").Preload("PortfolioRecords.Project.Client").Where("id = ?", id).First(&user).Error
	return user, err
}
...
func (u userRepository) FindUserByEmail(email string) (user models.User, err error) {
	err = u.DB.Preload(clause.Associations).Preload("PortfolioRecords.Technologies").Preload("Profile.Technologies").Preload("PortfolioRecords.Project.Client").Where("email = ?", email).First(&user).Error
	return user, err
}
